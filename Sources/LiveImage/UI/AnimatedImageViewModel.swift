...
@MainActor
final class AnimatedImageViewModel: Sendable {
        ...
        task = Task.detached(priority: configuration.taskPriority) { [image, cache, configuration] in
            await withTaskCancellationHandler { [image, configuration] in
                @Sendable func makeAndCacheImage(
                    ...
                    print("makeAndCacheImage \(index) \(key)")
                    let image = autoreleasepool { image.makeImage(at: index) }
                ...
                let delayTimes = (0 ..< imageCount).map { index in autoreleasepool { image.makeDelayTime(at: index) } }
